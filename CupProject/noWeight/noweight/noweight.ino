...
int servoOpen = 0;
int servoClose = 30;
int servoPin = 10;    // ======================== CHANGE TO PIN 11 TO WORK ON DR.BVK'S BOARD ======================
...
cup adultcup(3.5,12.5,0.42,0.3,2.0,0.0); 
 
// Initialize the liquid as water.
// 200 ml of water in the basecup
liquid water(90,1,1,adultcup);
...
//    BLE.setLocalName("Nano 33 BLE Sense");
    ...
    Serial.begin(9600);
    // cupLiq.volume = realVol(); =============================================realVol() NOT DECLARED IN SCOPE 
    // Initializing the cup and determines instantaneous volume
    cupLiq.volume = min(basecup.volume(),cupLiq.volume);
    cupLiq.volume = max(0,cupLiq.volume);
    // cupAngle = realAng(cupAngle); ==========================================realAng(cupAngle) NOT DECLARED IN SCOPE
    // Initiate a time value
...
void loop() { 
  ...
  //Serial.print("cupAngle:  ");
  //Serial.println(cupAngle);
   Serial.println("running");
   // Pulled from the library takes the running volume and the current cup geometry and estimates the shape of the water based upon the 
   // where things are placed in terms of location of the water and once it reached the hole
   // Just figures out the variable not the flow
   cupLiq.hl_solve(basecup,cupAngle);

  // Once the head reaches the bottom of the drinking hole
  // Determine the head when the height exceeds the drinking aperature.
  cuphead=head(basecup,cupLiq,cupAngle);
  //Serial.print("cuphead:  ");
  //Serial.println(cupAngle);  
  // Q is the flow rate and Q_desired will be assigned to the HIGH MEDIUM AND LOW VALUES      
